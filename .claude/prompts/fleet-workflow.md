...
<!-- Placeholders fleet-commander-dirty, fleet-commander-dirty, main, {{ISSUE_NUMBER}} are replaced during installation. -->

# Diamond Workflow — fleet-commander-dirty
...
```
User: claude --worktree fleet-commander-dirty-{N}
...
1. If `git rebase origin/main` fails with conflicts → state Blocked
...
| Dev pushes without rebase | ALWAYS rebase on main before push |
